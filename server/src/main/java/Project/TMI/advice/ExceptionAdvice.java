...
import Project.TMI.model.CommonResult;
...
public class ExceptionAdvice {
    ...
    @ExceptionHandler(PasswordConfirmNotSameException.class)
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    protected CommonResult PasswordConfirmNotSameException(HttpServletRequest request, HttpServletResponse response, PasswordConfirmNotSameException e) {
        // 예외 처리의 메시지를 MessageSource에서 가져오도록 수정
        response.setStatus(400);
        return responseService.getFailResult(1234, "nooooooooooooooooooooooooooooooooo");
    }

    // status정보에 해당하는 메시지를 조회합니다.
